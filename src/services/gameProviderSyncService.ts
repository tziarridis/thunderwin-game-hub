...
export class GameProviderSyncService {
  ...
  async syncProviderGames(providerId: string): Promise<Game[]> {
    try {
      ...
      const mappedGames: Game[] = providerGames.map((game: ProviderGame) => ({
        id: '', // Generated by the database
        provider_id: parseInt(providerId) || 1, // Convert string to number or use default
        name: game.name,
        ...
        // Add required fields from Game type
        game_id: game.slug || game.name.toLowerCase().replace(/\s+/g, '-'),
        game_name: game.name,
        game_code: game.slug || '',
        has_lobby: false,
        has_jackpot: false,
