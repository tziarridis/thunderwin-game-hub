...
import { PRAGMATIC_PLAY_ERROR_CODES } from './pragmaticPlayConfig';
...
export const pragmaticPlayTransactionHandler = {
   ...
   */
  processTransaction: async (config, data) => {
  ...
  validateHash: (data, secretKey) => {
    try {
      ...
      const orderedData = Object.keys(dataWithoutHash).sort().reduce((obj, key) => {
        obj[key] = dataWithoutHash[key];
        return obj;
      }, {});
      ...
      // Generate MD5 hash - using our browser-compatible implementation
   ...
   */
  isTransactionProcessed: async (trxId) => {
   ...
   */
  getPlayerBalance: async (playerId, currency) => {
   ...
   */
  updatePlayerBalance: async (playerId, newBalance, currency) => {
