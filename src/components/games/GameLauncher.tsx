...
const GameLauncherPage: React.FC = () => { // Renamed component to avoid conflict if another GameLauncher exists
  ...
  const [_isLoading, setIsLoading] = useState(true); // Renamed to avoid conflict with a different isLoading
  const [_error, setError] = useState<string | null>(null); // Renamed to avoid conflict
  ...
  const fetchGameAndLaunchUrl = useCallback(async (mode: 'real' | 'demo') => {
    ...
    try {
      ...
      if (!fetchedGame) {
        fetchedGame = await gameService.getGameBySlug(gameSlug, providerSlug);
        if (!fetchedGame) {
  ...
  useEffect(() => {
    const fetchInitialGameData = async () => {
        ...
        } else if (game) { // If game is already loaded (e.g. from state), ensure loading is false
            setIsLoading(false);
        }
    };
    fetchInitialGameData();
  }, [gameSlug, providerSlug, game]); // game dependency ensures this runs if game becomes null
  ...
  if (!game) {
     return ( 
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] p-4">
        { !_isLoading && <Info className="h-16 w-16 text-muted-foreground mb-4" /> }
        { _isLoading ? <Loader2 className="h-16 w-16 animate-spin text-primary mb-4" /> : <p className="text-lg text-muted-foreground">Game data not available.</p>}
          ...
          <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start">
            ...
            <div className="w-full md:w-2/3 space-y-4">
              ...
              <div className="pt-4 space-y-3">
                {_isLoading && <div className="flex items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-primary" /> <span className="ml-2">Loading launch options...</span></div>}
                
                {_error && !_isLoading && <p className="text-destructive text-sm flex items-center"><AlertTriangle className="h-4 w-4 mr-2" /> {_error}</p>}
