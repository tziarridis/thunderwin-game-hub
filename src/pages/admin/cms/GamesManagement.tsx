...
const CMSGamesManagement = () => {
  const { games, loading, error, fetchGames /*, totalGames */ } = useGames();
  // Commented out totalGames as it might not exist on useGames context
  ...
  const { launchGame: launchPreviewGame } = useGames(); // Renamed to avoid conflict if another launchGame is in scope

  const handleLaunchPreview = async (game: Game) => {
    try {
      const launchOptions: GameLaunchOptions = { // Explicitly type launchOptions
          mode: "demo",
          playerId: "cms_preview_user",
          currency: "USD",
          language: "en",
          platform: "web",
          returnUrl: window.location.href
      };
      const gameUrl = await launchPreviewGame(game, launchOptions);
      if (gameUrl) {
        window.open(gameUrl, "_blank");
        toast.success(`Launching preview for ${game.title}`);
      } else {
        toast.error("Could not launch game preview.");
      }
    } catch (err) {
      toast.error(`Failed to launch game preview: ${(err as Error).message}`);
    }
  };

  if (loading) return <div className="p-4">Loading games...</div>;
  if (error) return <div className="p-4 text-red-500">Error: {error.toString()}</div>;

  return (
    <div className="p-6 bg-gray-900 min-h-screen text-white">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Manage Games</h1>
        {/* <p className="text-sm text-gray-400">Total Games: {totalGames}</p> */}
            ...
            {filteredGames.map((game) => (
              <TableRow key={game.id} className="border-gray-700 hover:bg-gray-750">
                ...
                <TableCell className="text-right">
                  ...
                  <Button variant="outline" size="sm" onClick={() => handleLaunchPreview(game)}>Preview</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
...
export default CMSGamesManagement;
